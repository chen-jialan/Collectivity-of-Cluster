...
class Population:
    def __init__(self, ratio_CO=0.5, adjust_value=-2, pressure=1e5,
                 standard_pressure=100000, ads_number=10, cycle_number=1, temperature=300,
                 cutoff_distance=1.95, cutoff_distance1=1.2, cutoff_distance2=2.7, cutoff_distance3=1.3,
                 potential_O2=-1.071316E+01/2, potential_CO=-15.78612, potential_CO2=-24.24603, all_CO=20, all_O2=20,
                 volume=1000):
        self.ratio_CO = ratio_CO
        self.ads_number = ads_number
        self.cycle_number = cycle_number
        self.ConstantR = 8.618E-5
...
# --------------badstr-cluster----------------------------------------------------------------------------------------------------------------------
    ...
    def bad2(self, structure):
        ...
        for pot in structure_dis.keys():
            ...
            if 'Cu-' in pot:
                if 'O' in pot_1:
                    if dis_min >= 5 or dis_min < 1.3:
                        a += 1

                elif 'Cu-' in pot_1:
                    if dis_min >= 5 or dis_min < 1.5:
                        a += 1
                ...
                if 'Cu-' in pot_1:
                    if dis_min >= 4 or dis_min < 1.3:
                        a += 1

                elif 'O' in pot_1:
                    if dis_min >= 3 or dis_min < 1.1:
                        a += 1
...
# --------------del-cluster---------------------------------------------------------------------------------------------------
    def del_cluster(self, cluster, allatom, cluster_gas):
        ...
        #dis_ini = 0
        # for pot_Cu in potCu:
        #    if float(cluster_dis[pot_Cu][2]) >dis_ini:
        #        dis_ini = float(cluster_dis[pot_Cu][2])
        #        pot_max = pot_Cu
        #cluster_dis[pot_max] = cluster[atom]
        #allatom_dis[pot_max] = cluster[atom]
        # if not 'none' in cluster_gas_dis[pot_max]:
        #    for pot_gas in cluster_gas_dis[pot_max]:
        #        del allatom_dis[pot_gas]
        #cluster_gas_dis[pot_max] = 'none'
        # cluster_left = []
        # cluster_right = []
        # for i in range(1):
        #     for pot in cluster_dis.keys():
        #         if cluster_dis[pot][0]<atom_dis[0]:
        #             cluster_left.append(pot)
        #         else:
        #             cluster_right.append(pot)
        #     if len(cluster_left) >=1 and len(cluster_right) >=1:
        #         ini = 100
        #         for pot_left in cluster_left:
        #             for pot_right in cluster_right:
        #                 dis = np.linalg.norm(np.array(cluster_dis[pot_left]) - np.array(cluster_dis[pot_right]))
        #                 if dis < ini:
        #                     ini = dis
        #                     pot_r = pot_right
        #                     pot_l = pot_left
        #         if ini > 3:
        #             if len(cluster_right) < len(cluster_left):
        #                 arr_atom_to_cluster=(np.array(cluster_dis[pot_r])- np.array(cluster_dis[pot_l]))/ini*self.cutoff_distance2
        #                 for pot_r in cluster_right:
        #                     cluster_dis_post = copy.deepcopy(np.array(cluster_dis[pot_r]))
        #                     cluster_dis[pot_r] = list(np.array(cluster_dis[pot_l]) + arr_atom_to_cluster)
        #                     allatom_dis[pot_r] = cluster_dis[pot_r]
        #                     distance_all = np.array(cluster_dis[pot_r]) - cluster_dis_post
        #                     if not 'none' in cluster_gas_dis[pot_r]:
        #                         for pot_gas in cluster_gas_dis[pot_r]:
        #                             cluster_gas_dis[pot_r][pot_gas] = list(np.array(cluster_gas_dis[pot_r][pot_gas]) + distance_all)
        #                             allatom_dis[pot_gas] = list(np.array(cluster_gas_dis[pot_r][pot_gas]) + distance_all)
        #             else:
        #                 arr_atom_to_cluster=uniform(1,1.2)*(np.array(cluster_dis[pot_l])- np.array(cluster_dis[pot_r]))/ini*self.cutoff_distance2
        #                 for pot_r in cluster_left:
        #                     cluster_dis_post = copy.deepcopy(np.array(cluster_dis[pot_l]))
        #                     cluster_dis[pot_l] = list(np.array(cluster_dis[pot_r]) + arr_atom_to_cluster)
        #                     allatom_dis[pot_l] = cluster_dis[pot_l]
        #                     distance_all = np.array(cluster_dis[pot_l]) - cluster_dis_post
        #                     if not 'none' in cluster_gas_dis[pot_l]:
        #                         for pot_gas in cluster_gas_dis[pot_l]:
        #                             cluster_gas_dis[pot_l][pot_gas] = list(np.array(cluster_gas_dis[pot_l][pot_gas]) + distance_all)
        #                             allatom_dis[pot_gas] = list(np.array(cluster_gas_dis[pot_l][pot_gas]) + distance_all)
        #         else:
        #             break
        #     else:
        #         break
...
# ------------- MC energy to get true str ---------------------------------------------------------------------------------------------------
    def energy_estimate(self, energy, ini_energy):
        ...
        if energy <= ini_energy:
            ...
            probability = exp(-1/(self.ConstantR *
                                  self.temperature)*(energy - ini_energy))
...
# ------------- MC energy to get true str of add operation--------------------------------------------------------------------------------------
    def energy_estimate_add(self, energy, ini_energy, mass_e, N_exchange=0):
        a = 1
        if energy <= ini_energy:
            a = 0
        else:
            V = self.volume*1E-30
            Lammba = self.h/sqrt(2 * pi *
                                 mass_e * self.kB * self.temperature)
            adjust = V/(N_exchange*math.pow(Lammba, 3))
            probability = adjust * \
                exp(-1/(self.ConstantR*self.temperature)
                    * (energy - ini_energy))
            if probability > uniform(0, 1):
                a = 0
            else:
...
# ------------- MC energy to get true str of del operation--------------------------------------------------------------------------------------
    def energy_estimate_del(self, energy, ini_energy, mass_e, N_exchange=0):
        a = 1
        if energy <= ini_energy:
            a = 0
        else:
            V = self.volume*1E-30
            Lammba = self.h/sqrt(2 * pi * mass_e *
                                 self.kB*self.temperature)
            adjust = (N_exchange*math.pow(Lammba, 3))/V
            probability = adjust * \
                exp(-1/(self.ConstantR*self.temperature)
                    * (energy - ini_energy))
            if probability > uniform(0, 1):
...
# --------------ini --------------
    ...
    if len(gas_atom_ini) != 0:
        ...
        num_atom_O = 0
    N_exchange = num_atom_c + num_atom_O
            ...
            # ----------------------eann --------------------------------
                ...
                #    if number_CO2 == 0 :
                    ...
                    if ration_final == 0 and number_CO2 == 0 and a_bad == 0 and x_energy_bad == 0 and np.max(force_bad) <= 0.2:
                        ...
                        traj = Trajectory('allstr.traj', 'a')
                        traj.write(atoms)
                        print("gcmc operation is %s" % gcmc_operation)
                        if gcmc_operation == 0:
                            a = pop.energy_estimate(Energy, ini_energy)
                        elif gcmc_operation == 1:
                            a = pop.energy_estimate_add(
                                Energy, ini_energy, mass_e=30, N_exchange=0)
                        elif gcmc_operation == 2:
                            a = pop.energy_estimate_del(
                                Energy, ini_energy, mass_e=30, N_exchange=0)
