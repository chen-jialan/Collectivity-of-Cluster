...
# ---------------------distance of site(xafas)-------------------


def distance_of_site(atom_all, site_all):
    sum_site_dis = dict()
    for pot in site_all.keys():
        print(pot)
        dis_Cu = []
        dis_O = []
        dis_C = []
        dis_ini = np.array(atom_all[pot])
        list_all = site_all[pot]
        for pot_next in list_all:
            dis_next = np.array(atom_all[pot_next])
            dis = float(np.linalg.norm(dis_ini-dis_next))
            if 'Cu' in pot_next:
                dis_Cu.append(dis)
            elif 'O' in pot_next:
                dis_O.append(dis)
                ...
                dis_C.append(dis)
        if len(dis_Cu) != 0:
            dis_Cu_all = sum(dis_Cu)/len(dis_Cu)
        else:
            dis_Cu_all = 0
        if len(dis_O) != 0:
            dis_O_all = sum(dis_O)/len(dis_O)
        else:
            dis_O_all = 0
        if len(dis_C) != 0:
            dis_C_all = sum(dis_C)/len(dis_C)
        else:
            dis_C_all = 0
        sum_site_dis[pot] = [dis_Cu_all, dis_O_all, dis_C_all]
    # print(sum_site_dis)
    return sum_site_dis
...
if __name__ == '__main__':
    ...
    parser.add_argument('--energy_files', nargs='+',
                        default=['energy_meta', 'energy_meta2'])
    parser.add_argument('--poscar_dirs', nargs='+',
                        default=['goodgas', 'dataall2'])
    parser.add_argument('--meta_str_dir', type=str, default='file_metastr')
    parser.add_argument('--temperature', type=float, default=400)
    parser.add_argument('--energy_cutoff', type=float, default=0.5)
    parser.add_argument('--H_fix', type=float, default=1.6)
    parser.add_argument('--atom_list', nargs='+',
                        default=['Cu', 'Ce(s)', 'O', 'C', 'O_s'])
    args = parser.parse_args()
    fileall1 = args.energy_files
    posfile = args.poscar_dirs
    meta_str_file = args.meta_str_dir
    ...
    energy_final = []
    file_final = []
    finds = ['findstr1']  # ----------find structure in meta---------
    ...
    # -----------------xafs dis_all------------------
    dis_all = []
    for i_all in range(1):
        P = []
        energy_all, number_O, number_CO, gcmc_energy_all, poscar = energy_all_str(
            path, fileall1[i_all])
        ...
        for energy_e in energy_all_copy:
            if energy_e - energy_min <= energy_cutoff and energy_e >= energy_min:
        ...
        for i in range(len(P_copy)):
            if P_copy[i] != 0:
                ...
                if os.path.exists('./%s/POSCAR-%s' % (posfile[i_all], int(poscar[number_index]))):
                    copyfile('./%s/POSCAR-%s' % (posfile[i_all], int(
                        poscar[number_index])), './%s/POSCAR-%s' % (finds[i_all], a))
                    copyfile('./%s/POSCAR-%s' %
                             (posfile[i_all], int(poscar[number_index])), 'POSCAR')
                ...
                # ----------------distance---------------
                site_dis = distance_of_site(allatom, site_all)
                ...
                for pot in atom_type.keys():
                    atom_Cu, dis = distance_site_site(
                        pot, atom_Cu_all, allatom)
                    atom_type_new = [atom_type[pot], atom_type[atom_Cu]]
                    if not atom_type_new in list_site_all:
                        print(a, energy_all_copy[i], P_copy[i], i_all, int(
                            poscar[number_index]))
                        if os.path.exists('./%s/POSCAR-%s' % (posfile[i_all], int(poscar[number_index]))):
                            copyfile('./%s/POSCAR-%s' % (posfile[i_all], int(
                                poscar[number_index])), './%s/POSCAR-%s' % (meta_str_file, int(poscar[number_index])))
                        list_site_all.append(atom_type_new)
                        list_site_num.append(P_copy[i])
                        index_poscar.append(int(poscar[number_index]))
                        dis_all.append(
                            [[site_dis[pot][0]], [site_dis[pot][1]], [site_dis[pot][2]]])
                    else:
                        index_site = list_site_all.index(atom_type_new)
                        list_site_num[index_site] += P_copy[i]
                        dis_all[index_site][0].append(site_dis[pot][0])
                        dis_all[index_site][1].append(site_dis[pot][1])
                        dis_all[index_site][2].append(site_dis[pot][2])
    ...
    for i in range(len(dis_all)):
        dis_fin_1 = []
        for j in range(len(dis_all[i])):
            a = np.array(dis_all[i][j])
            ava = a[np.nonzero(a)].mean()
            dis_fin_1.append(ava)
        dis_fin.append(dis_fin_1)
    dis_fin = list(np.array(dis_fin).transpose())
    # print(list_site_all,list_site_num)
    sum_of_site = []
    s = ['-', '~']
